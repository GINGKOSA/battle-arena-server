...
function makeHPCanvas(name, hp, maxHP, color) {
  ...
  ctx.fillText(name.slice(0, 14), W/2, 34);
  ctx.shadowBlur = 0;
  ...
  rrect(ctx, 14, 50, Math.max(0, (W-28) * pct), 22, 6); ctx.fill();
  ctx.shadowBlur = 0;

  ctx.fillStyle = 'rgba(230,230,255,.95)';
...
function initThree() {
  ...
  dl.position.set(5, 8, 5); dl.castShadow = true; s3.add(dl);
  const pl1 = new THREE.PointLight(0xff6600, 1.4, 10);
  pl1.position.set(-4, 2, 2); s3.add(pl1);
  const pl2 = new THREE.PointLight(0x00aaff, 1.4, 10);
  pl2.position.set(4, 2, 2); s3.add(pl2);
  s3._pl1 = pl1; s3._pl2 = pl2;

  // Sol
  const fl = new THREE.Mesh(
    new THREE.PlaneGeometry(20, 20, 8, 8),
    new THREE.MeshStandardMaterial({ color: 0x12122a, roughness: .8 })
  );
  ...
  const arena = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x1a1a3a, roughness: .5 })
  );
...
function buildChars() {
  ...
  G.players.forEach((p, i) => {
    ...
    mesh.rotation.y = i < Math.ceil(n / 2) ? 0.4 : Math.PI - 0.4;
    s3.add(mesh); meshes[p.slot] = mesh;

    const sp = makeSprite(p.pseudo, p.hp, p.maxHP, SLOT_CLR[p.slot] || '#fff');
    sp.position.set(x, 3.3, z || 0);
    s3.add(sp); hpSprites[p.slot] = sp;
  });
}
...
function makeChar(col, fire) {
  ...
  const body = new THREE.Mesh(new THREE.BoxGeometry(.7, .9, .5), m);
  body.position.y = 1.15; body.castShadow = true; g.add(body);

  const head = new THREE.Mesh(new THREE.SphereGeometry(.32, 16, 16), m);
  ...
  const emb = fire
    ? new THREE.Mesh(new THREE.SphereGeometry(.12, 8, 8),
        new THREE.MeshStandardMaterial({ color: 0xffcc00, emissive: 0xff8800, emissiveIntensity: .5 }))
    : new THREE.Mesh(new THREE.ConeGeometry(.1, .3, 8),
        new THREE.MeshStandardMaterial({ color: 0x88ddff, emissive: 0x0088ff, emissiveIntensity: .4 }));
  emb.position.set(0, fire ? 1.2 : 2.28, fire ? .28 : 0); g.add(emb);

  const sh = new THREE.Mesh(
    new THREE.CircleGeometry(.45, 16),
...
function spawnHitParticles(slot) {
  ...
  for (let i = 0; i < 12; i++) {
    const m = new THREE.Mesh(
      new THREE.SphereGeometry(.06 + Math.random() * .06, 6, 6),
